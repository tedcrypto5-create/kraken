...
# 1. Create the specific directory tree
mkdir -p accounts/_next/static/css 
mkdir -p accounts/_next/static/chunks/pages 
mkdir -p accounts/_next/static/accounts-_bvWjgB7JhwiuX5g_V31E

# 2. Download CSS
curl -L "$BASE/accounts/_next/static/css/637574239dfd98ee.css" -o "accounts/_next/static/css/637574239dfd98ee.css"

# 3. Download Logic Chunks
...
for c in "${CHUNKS[@]}"; do 
    curl -L "$BASE/accounts/_next/static/chunks/$c" -o "accounts/_next/static/chunks/$c"
done

# 4. Download Page & Manifests
...
# 5. FIX FOR BLANK PAGE (ADDITIVE ONLY)
# This copies index.html to 404.html so that sub-paths like /sign-in still load your code
